...
if server_flag:
    data_handler.load_data_general("/home/tkdrlf9202/data-Daimler_Extracted/images",
                                   im_val, dummy, file_name, format='png', label=0, datasize=21790)
else:
...
for idx, sample in enumerate(samples):
    ...
    fp_pi, miss_rate_pi = detect_module.generate_bounding_boxes(model=model, image=sample, file_name=file_name[idx],
                                                                downscale=1.2, step=11, min_height=96, min_width=48,
                                                                grd_truth_path=ground_truth_path, trainingFlag=0,
